...
/**
 * The "✳ Claude" wordmark (Anthropic's lockup), bundled as inline SVG so it
 * needs no network and scales to the surrounding font size. The burst keeps the
 * brand orange; the wordmark uses `currentColor` so it stays legible on any
 * button background. Sized via `1em` height, drop it inline where "Claude"
 * would otherwise be typed.
 */
export function ClaudeWordmark({ style, ...props }: SVGProps<SVGSVGElement>): React.JSX.Element {
  return (
    <svg
      viewBox="0 0 689.97997 148.17999"
      role="img"
      aria-label="Claude"
      // Inline height/width beat the Button primitive's [&_svg]:size-* rules,
      // which would otherwise squash this 4.66:1 wordmark into a square.
      style={{ height: '0.9em', width: 'auto', ...style }}
      {...props}
    >
