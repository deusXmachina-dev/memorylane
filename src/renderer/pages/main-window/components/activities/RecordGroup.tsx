...
function groupLabel(first: RecentActivity): string {
  const main = first.windowTitle?.trim() || first.summary?.trim() || first.appName
  return main
}

function groupSubtitle(first: RecentActivity): string {
...
export function RecordGroup({ activities }: RecordGroupProps): React.JSX.Element {
  ...
  const dedupedSummaries: { activity: RecentActivity; summary: string }[] = []
