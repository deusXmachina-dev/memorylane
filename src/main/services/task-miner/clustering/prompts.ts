...
You will receive a JSON object with "clusters". Each has an id, an existing "label" (may be empty), "stats" computed from ALL members (times_seen, span_days from first to last sighting, median_active_min), and member sightings (title, subject (the specific thing that run acted on), description, apps (app identities: the website host for web work, e.g. "dashboard.stripe.com", the application name for desktop work, e.g. "Ghostty"), "active_min" (measured active time; pauses under ${BRIDGE_MAX_GAP_MIN} minutes count as continued work), date, and "steps" — that run's observed happy path). Members sharing a title but differing in subject are the same process run on different subjects, direct evidence for adopting that shared title as the label.
...
2. CLASSIFY: this is where eliminability is decided — the sightings were mined as tasks, with no view on whether anything could take them over. You have what a single run does not: how often it recurs, how long it takes, and how much it varies. "kind" is one of:
   ...
   For "procedure" — and for "monitoring" that is genuinely alert-replaceable — also output "mechanism": ONE sentence naming the concrete replacement (a script, integration, alert, platform feature, or process change), derived from what the member sightings' descriptions and steps actually show — never invent a mechanism the evidence doesn't support. No concrete mechanism = not eliminable: omit the field. For "ambient", "dev-loop" and "judgment" NEVER write a mechanism — whatever you write there is stored verbatim and shown to the user as a promise that this work can be taken over.
...
\`\`\`json
{
  "clusters": [
    ...
    { "id": "<cluster id>", "label": "...", "description": "...", "kind": "monitoring", "mechanism": "An alert on the watched condition that ...", "steps": ["dashboard.stripe.com: ...", "..."], "variables": ["..."] },
    { "id": "<cluster id>", "label": "...", "description": "...", "kind": "judgment", "steps": ["...", "..."], "variables": ["..."] }
  ]
}
\`\`\``
}
