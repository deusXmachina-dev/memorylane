...
- "clusters": clusters needing review. Each has an id, a "new" flag (true = created just now by an automatic grouping pass), an existing "label" (may be empty), "stats" computed from ALL members (times_seen, span_days from first to last sighting, median_active_min), and sample member sightings (title, description, apps, interaction minutes, date).
