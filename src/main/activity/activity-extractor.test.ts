...
describe('ActivityExtractor', () => {
  ...
  it('holds dispatch while isReady is false: nothing persisted, nothing acked', async () => {
    const persisted: string[] = []
    const { extractor, activityStream, consumerId } = createExtractor({
    ...
    await extractor.start()
    await activityStream.append(makeActivity('gated-1', 1_000))
    await activityStream.append(makeActivity('gated-2', 2_000))

    await sleep(50)
    expect(persisted).toEqual([])
    expect(extractor.getStats().queued).toBe(2)
    expect(await activityStream.getAck(consumerId)).toBeNull()
  })

  it('kick() after the gate opens drains queued tasks in order', async () => {
    let ready = false
    const persisted: string[] = []
    const { extractor, activityStream, consumerId } = createExtractor({
      config: { isReady: () => ready, maxConcurrent: 1 },
      sink: {
        persist: async ({ activity }) => {
          persisted.push(activity.id)
        },
    ...
    await extractor.start()
    await activityStream.append(makeActivity('held-1', 1_000))
    await activityStream.append(makeActivity('held-2', 2_000))
    await sleep(50)
    expect(persisted).toEqual([])
    ...
    await waitFor(() => persisted.length === 2, 'Expected queued tasks to drain after kick')
    expect(persisted).toEqual(['held-1', 'held-2'])
    await waitFor(
      async () => (await activityStream.getAck(consumerId)) === 1,
      'Expected ack to advance after the gate opened',
    )
  })

  it('stop() does not hang on gate-held tasks', async () => {
    const { extractor, activityStream } = createExtractor({
      config: { isReady: () => false },
    })

    await extractor.start()
    await activityStream.append(makeActivity('never-run', 1_000))
    await sleep(20)
    ...
    await expect(
      Promise.race([
        extractor.stop(),
        sleep(1_000).then(() => {
          throw new Error('stop() hung on gated tasks')
        }),
      ]),
    ).resolves.toBeUndefined()
    expect(extractor.getStats().succeeded).toBe(0)
  })
})
