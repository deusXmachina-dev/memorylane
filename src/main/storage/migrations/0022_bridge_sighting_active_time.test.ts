...
describe('0022_bridge_sighting_active_time', () => {
  const TEST_DB_PATH = path.join(os.tmpdir(), 'temp_migration_0022_test.db')
  ...
  it('bridges a short gap and excludes a long one', () => {
    addActivity('a1', 0, 2)
    addActivity('a2', 4, 6) // 2-min gap — bridged
    addActivity('b1', 0, 2)
    addActivity('b2', 32, 34) // 30-min gap — excluded
    addSighting('short-gap', ['a1', 'a2'], 4)
    addSighting('long-gap', ['b1', 'b2'], 4)

    runMigration()

    expect(activeMin('short-gap')).toBe(6)
  ...
  it('never exceeds the wall-clock span', () => {
    addActivity('c1', 0, 5)
    addActivity('c2', 8, 20)
    addActivity('c3', 90, 95)
    addSighting('mixed', ['c1', 'c2', 'c3'], 22)

    runMigration()

    const s = storage.sightings.getById('mixed')!
    expect(s.interactionMin).toBe(25)
