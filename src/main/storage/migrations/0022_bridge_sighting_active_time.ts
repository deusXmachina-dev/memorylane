...
// numbers, so neither the threshold nor the union rule follows the miner's.
...
function bridgedActiveMin(activities: ActivityRow[]): number {
...
export const migration: Migration = {
  ...
  up(db: Database.Database): void {
    ...
    for (const row of rows) {
      ...
      update.run(bridgedActiveMin(resolved), row.id)
    }
  },
}
